...
<ul class="list-group">
    ...
    @if($order->status === 'preparing')
        <form action="{{ route('orders.updateStatus') }}" method="POST" class="w-50">
            ...
            <input type="hidden" name="redirect_url" value="{{ route('runnerstatuspage', ['id' => $order->id]) }}">
            <button type="submit" class="btn w-100 fw-bold py-2 rounded-pill shadow-sm"
                ...
                <i class="fas fa-check-circle"></i> Accept Pickup Request
            </button>
        </form>

        <form action="{{ route('runner.runnerhomepage') }}" class="w-50">
            @csrf
            <button type="submit" class="btn w-100 fw-bold py-2 rounded-pill shadow-sm"
    ...
    @else
        <form action="{{ route('runnerhistorypage') }}" class="w-100">
